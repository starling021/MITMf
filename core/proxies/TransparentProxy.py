...
formatter = logging.Formatter("%(asctime)s [TProxy] %(clientip)s [type:%(browser)s-%(browserv)s os:%(clientos)s] %(message)s", datefmt="%Y-%m-%d %H:%M:%S")
log = logger().setup_logger('TransparentProxy', formatter)
...
class TransparentProxy(controller.Master):
